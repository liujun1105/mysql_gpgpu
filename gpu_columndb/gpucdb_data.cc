...
*/
#include "gpucdb_data.h"
#include <my_dir.h>
#include <string.h>

Gpucdb_data::Gpucdb_data(void)
{
  data_file = -1;
  number_records = -1;
...
int Gpucdb_data::create_table(char *path)
{
  DBUG_ENTER("GpucdbIndex::create_table");
  open_table(path);
  number_records = 0;
...
int Gpucdb_data::open_table(char *path)
{
  DBUG_ENTER("Gpucdb_data::open_table");
  /*
...
long long Gpucdb_data::write_row(uchar *buf, int length)
{
  long long pos;
  int i;
  int len;
  ...
  DBUG_ENTER("Gpucdb_data::write_row");
  /*
...
long long Gpucdb_data::update_row(uchar *old_rec, uchar *new_rec,
                                   int length, long long position)
{
  ...
  DBUG_ENTER("Gpucdb_data::update_row");
  if (position == 0)
...
int Gpucdb_data::delete_row(uchar *old_rec, int length, 
                             long long position)
{
  int i = -1;
  long long pos;
  ...
  DBUG_ENTER("Gpucdb_data::delete_row");
  if (position == 0)
    position = header_size; //move past header
  pos = position;
  /* 
...
int Gpucdb_data::read_row(uchar *buf, int length, long long position)
{
  int i;
  int rec_len;
  long long pos; 
  ...
  DBUG_ENTER("Gpucdb_data::read_row");
  if (position <= 0)
    position = header_size; //move past header
...
/* close file */
int Gpucdb_data::close_table()
{
  DBUG_ENTER("Gpucdb_data::close_table");
  if (data_file != -1)
  {
...
int Gpucdb_data::records()
{
  DBUG_ENTER("Gpucdb_data::num_records");
  DBUG_RETURN(number_records);
}

/* return number of deleted records */
int Gpucdb_data::del_records()
{
  DBUG_ENTER("Gpucdb_data::num_records");
...
int Gpucdb_data::read_header()
{
  int i;
  int len;

  DBUG_ENTER("Gpucdb_data::read_header");
  if (number_records == -1)
  {  
...
int Gpucdb_data::write_header()
{
  int i;

  DBUG_ENTER("Gpucdb_data::write_header");
  if (number_records != -1)
  {
    my_seek(data_file, 0l, MY_SEEK_SET, MYF(0));
...
long long Gpucdb_data::cur_position()
{
  long long pos;

  DBUG_ENTER("Gpucdb_data::cur_position");
...
int Gpucdb_data::trunc_table()
{
  DBUG_ENTER("Gpucdb_data::trunc_table");
  if (data_file != -1 )
  {
...
int Gpucdb_data::row_size(int length)
{
  DBUG_ENTER("Gpucdb_data::row_size");
